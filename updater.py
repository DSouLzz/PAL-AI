...
def _github_latest_release(repo):
    ...
    r = requests.get(api_url, headers={"Accept": "application/vnd.github+json", "User-Agent": "PAL-AI-Updater"}, timeout=10)
...
def check_for_update():
    ...
    return {"status": "update_available", "current": current, "latest": latest, "download_url": zip_url, "sha256": expected, "notes": release.get("body", "") or release.get("name", "") or ""}
...
def prepare_update(download_url, latest_version, expected_sha256):
    ...
    if actual != expected:
        try:
            ...
            pass
        raise ValueError(f"SECURITY CHECK FAILED: SHA-256 mismatch. Expected {expected}, received {actual}. The update was NOT installed.")
    with zipfile.ZipFile(zip_path, "r") as z:
        z.extractall(extract_dir)
    children = list(extract_dir.iterdir())
    payload = children[0] if len(children) == 1 and children[0].is_dir() else extract_dir
...
if py.exists() and req.exists(): subprocess.run([str(py), "-m", "pip", "install", "-r", str(req)], cwd=str(dst), check=False)
start = dst / "start.bat"
if start.exists(): subprocess.Popen(["cmd", "/c", "start", "", str(start)], cwd=str(dst))
